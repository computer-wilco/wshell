...
function loadTerminal() {
    ...
    function executeCommand(command, disableOutput = false) {
        ipcRenderer.send('execute-command', command, disableOutput);
    }

    term.reset();
    executeCommand('sh -c "$(cat ' + import.meta.dirname + '/doedingen.sh)"\r', true);
